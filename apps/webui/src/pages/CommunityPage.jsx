...
export default function CommunityPage() {
  ...
  // 收藏夹选择器状态（点击收藏按钮时显示）
  const [showCollectionSelector, setShowCollectionSelector] = useState(null); // 存储要操作的卡片ID
    ...
    <section className="panel panel-community">
      <div className="community-layout">
        <div className="community-header">
          <div>
            <div className="community-title">社区</div>
        ...
        <div className="community-content">
          {showFavorites ? (
                          ...
                          <div className="collection-item-info">
                            <div className="collection-item-name">
                              ...
                              {collection.isDefault && (
                                <span className="collection-item-default-badge">默认</span>
                              )}
                            </div>
              ...
              <div className="collection-content">
                {selectedCollectionId ? (
                  loadingCollection ? (
                    ...
                    <div className="collection-empty">该收藏夹还没有卡片</div>
                  )
                ) : (
                  <div className="collection-empty">
                    {app.communitySession ? "选择一个收藏夹查看内容" : "登录后可查看收藏夹"}
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="community-list manage-cards">
              ...
              <div className="manage-empty">未找到社区卡片。</div>
            )}
            </div>
          )}
        </div>
      ...
      {showCollectionManager && (
        <CollectionManager
          ...
          publicCollections={app.publicCollections || []}
          loading={app.collectionsLoading || false}
          onCreateCollection={async (name, description, visibility) => {
            if (!app.communitySession) {
              alert("请先登录");
              return { ok: false };
            }
            return app.createCollection?.(name, description, visibility);
          }}
          onUpdateCollection={async (id, updates) => {
            return app.updateCollection?.(id, updates);
          }}
          onDeleteCollection={async (id) => {
            return app.deleteCollection?.(id);
          }}
