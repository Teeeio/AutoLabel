...
function hydrateCard(card) {
  ...
  return {
    ...
    source: card.source || "bilibili" // 默认为bilibili
  };
}
 ...
 * @returns {Promise<Array>} 带验证结果的卡片数组
 */
...
// 本地视频播放器组件 - B站风格（完整复制B站注入滑块功能）
function LocalVideoPlayer({ localPath, localVideoRef, setDuration, setIsPlaying, syncCardRange, activeCard }) {
  ...
  // 移除zoomScale和showZoomBadge的useState，改用ref管理（完全复制B站）
  const [showZoomBadge, setShowZoomBadge] = useState(false);

  // Refs - 完全复制B站的拖动状态
  const timelineRef = useRef(null);
  const playerSurfaceRef = useRef(null);
  const timelineBarRef = useRef(null); // 进度条元素的ref
  const timelineClipRef = useRef(null); // 选区与滑块层，需跟随缩放
  ...
  const currentTimeRef = useRef(currentTime); // 播放头更新用ref
  ...
  // 滑块位置ref（RAF中直接更新DOM，避免触发React重渲染）
  ...
  useEffect(() => {
    if (activeCard) {
      ...
      // 如果当前播放位置不在新区间内，修正到新区间的起始位置
      if (localVideoRef.current) {
        const current = localVideoRef.current.currentTime;
        if (current < nextStart || current > nextEnd) {
          console.log('[范围变化] 播放位置', current, '不在新区间内 [', nextStart, ',', nextEnd, ']，修正到', nextStart);
  ...
  // range 更新时同步 slider 位置，避免拖动读取旧值
  useEffect(() => {
  ...
  useEffect(() => {
    console.log('[LocalVideoPlayer] 加载视频文件:', localPath);
    setIsLoading(true);
    setError(null);

    // 通过 IPC 请求主进程读取视频文
    window.localVideo?.load(localPath).then((arrayBuffer) => {
      console.log('[LocalVideoPlayer] 收到视频数据，大小', arrayBuffer.byteLength);

      // 创建 Blob URL
      const blob = new Blob([arrayBuffer], { type: 'video/mp4' });
      const url = URL.createObjectURL(blob);
      console.log('[LocalVideoPlayer] 创建 Blob URL:', url);

      setVideoUrl(url);
      setIsLoading(false);
    }).catch((error) => {
      console.error('[LocalVideoPlayer] 加载视频失败:', error);
  ...
  const applyZoomScaleToDOM = useCallback((scale, immediate = false) => {
    ...
    if (zoomActive && zoomBadgeRef.current) {
      zoomBadgeRef.current.textContent = `Zoom x${nextScale.toFixed(1)}`;
    }

    // ?? 关键：滑块位置始终用未缩放的像素值，因为滑块在缩放层内
      // 会自动跟随父元素的transform
    const xS = (rangeStart / duration) * baseWidth;
    const xE = (rangeEnd / duration) * baseWidth;
  ...
  const seekPlayer = useCallback(
    (timeValue) => {
      ...
      console.log('[seekPlayer] 定位', clamped, '(原始:', timeValue, ')');
    },
    [rangeStart, rangeEnd]
  );
  ...
  const safePlay = useCallback(async () => {
    ...
    try {
      ...
      console.error('[LocalVideoPlayer] 播放失败:', err);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (event) => {
      ...
      if (event.key === " " || event.code === "Space" || event.key === "ArrowRight" || event.key === "ArrowLeft") {
        event.preventDefault();
        event.stopPropagation();
      }
      if (!localVideoRef.current || !videoUrl) return;
      ...
      if (event.key === "ArrowRight" || event.key === "ArrowLeft") {
        if (keyHoldLocalRef.current.key && keyHoldLocalRef.current.key !== event.key) return;
        if (keyHoldLocalRef.current.key === event.key) return;
        keyHoldLocalRef.current.key = event.key;
        keyHoldLocalRef.current.long = false;
    ...
    const handleKeyUp = (event) => {
      if (playerActiveRef.current && (
        event.key === "ArrowRight" || event.key === "ArrowLeft" || event.key === " " || event.code === "Space"
      )) {
        event.preventDefault();
        event.stopPropagation();
      }
  ...
  useEffect(() => {
    ...
    const applyDragAtX = (clientX) => {
      ...
      // ?? 关键：滑块位置用未缩放的像素值（滑块在缩放层内，会自动跟随transform）
    ...
    const handleMove = (event) => {
      ...
      // 更新视觉压缩/拉伸效果（直接操作DOM，不触发React重渲染）
  ...
  }, [duration, clampNow, syncCardRange, applyAutoZoom]); // 依赖稳定回调与ref读取的最新
  useEffect(() => {
    if (!showZoomBadge) return;
    if (zoomBadgeRef.current) {
      zoomBadgeRef.current.textContent = `Zoom x${zoomStateRef.current.zoomScale.toFixed(1)}`;
    }
    updateZoomBadgePosition();
  ...
  // 同步currentTime到ref（供播放头RAF循环使用
  useEffect(() => {
    currentTimeRef.current = currentTime;
  }, [currentTime]);

  // 播放头位置更新（使用RAF持续更新，但不依赖currentTime避免频繁重建
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    // 如果当前播放位置不在区间内，修正到rangeStart
    if (localVideoRef.current && duration > 0) {
      const current = localVideoRef.current.currentTime;
      if (current < rangeStart || current > rangeEnd) {
        console.log('[位置修正] 当前时间', current, '不在区间[', rangeStart, ',', rangeEnd, ']，修正到', rangeStart);
  ...
  return (
    ...
    >
      ...
      <video
        ...
        onLoadedMetadata={() => {
          if (localVideoRef.current) {
            console.log('[本地视频] onLoadedMetadata - 时长:', localVideoRef.current.duration);
            ...
            // 自动播放
            console.log("[本地视频] ?? 开始自动播放");
            localVideoRef.current.currentTime = rangeStart;
            localVideoRef.current.play().then(() => {
              console.log('[本地视频] 自动播放成功');
              setIsPlayingState(true);
              setIsPlaying(true);
            }).catch(err => {
              console.error('[本地视频] 自动播放失败:', err);
            });
          }
        }}
        onTimeUpdate={() => {
        ...
        onPlay={() => {
          console.log('[本地视频] onPlay');
          setIsPlayingState(true);
          setIsPlaying(true);
        }}
        onPause={() => {
          console.log('[本地视频] onPause');
          setIsPlayingState(false);
          setIsPlaying(false);
        }}
        onError={(e) => {
          console.error('[本地视频] onError:', e);
        }}
      />
      ...
      <div
        ...
        style={{
          ...
          left: 0,
          right: 0,
          background: 'linear-gradient(to top, rgba(12, 14, 18, 0.96) 0%, rgba(12, 14, 18, 0.8) 50%, transparent 100%)',
          padding: '10px',
          display: 'flex',
          flexDirection: 'column',
          gap: '8px',
      ...
      >
        ...
        >
          ...
          <div
            ...
            style={{
              ...
              left: 0,
              right: 0,
              height: isScrubbing ? '12px' : '8px',
              top: '50%',
              marginTop: isScrubbing ? '-6px' : '-4px', // 垂直居中补偿
              borderRadius: '999px',
              background: '#8f98a4',
              zIndex: 1,
              transition: isScrubbing ? 'none' : 'height 0.12s ease',
              // transform 由 applyZoomScaleToDOM 通过 setProperty 动态控制
              transformOrigin: 'left center',
              overflow: 'visible' // 允许滑块超出进度条范围
            }}
          >
          </div>
            ...
            >
              ...
              <div
                ...
                style={{
                  ...
                  // left width JS 动态控制（像素值）
                  zIndex: 1,
                  pointerEvents: 'none'
                }}
              />

              {/* 开始时间滑块- B站风格：top: -10px（在进度条上方） */}
              <div
                ref={startHandleRef}
                ...
                style={{
                  ...
                  // left JS 动态控制（像素值）
                  zIndex: 2,
                  // CSS变量系统：--clip-scale由applyZoomScaleToDOM直接设置
          ...
          {/* 播放- 使用像素定位，避免受timelineBar transform影响 */}
          <div
            ref={playheadRef}
            data-role="playhead"
            ...
            style={{
              ...
              boxShadow: '0 0 0 4px rgba(64, 196, 255, 0.22)',
        ...
        >
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            ...
            <div className="player-time" style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '13px' }}>
...
export default function App() {
  ...
  const [previewCurrentTime, setPreviewCurrentTime] = useState(new Map()); // 跟踪每个预览的当前时
  const [isDraggingProgress, setIsDraggingProgress] = useState(false); // 跟踪是否正在拖动进度
  ...
  const localVideoRef = useRef(null); // 本地视频播放器ref
  const timelineRef = useRef(null);
  ...
  const [localVideoInfo, setLocalVideoInfo] = useState(null); // 本地视频元数据
  const [searchSourceType, setSearchSourceType] = useState("bilibili"); // 'bilibili' | 'local'
  const [selectedLocalFile, setSelectedLocalFile] = useState(null); // 选中的本地文件
  const [localVideoFolder, setLocalVideoFolder] = useState(null); // 当前选中的文件夹
  const [localVideoList, setLocalVideoList] = useState([]); // 文件夹中的视频列表
  ...
  useEffect(() => {
    const loadLastFolder = async () => {
      ...
      if (lastFolder) {
        console.log("[本地视频] 自动打开上次文件夹", lastFolder);
        ...
        try {
          ...
          if (scanResult?.ok && scanResult.files) {
            ...
            console.log("[本地视频] 自动加载文件夹, 共", scanResult.files.length, "个视频文件");
          }
        } catch (error) {
          console.error("[本地视频] 自动加载文件夹失败", error);
          // 如果文件夹不存在或无法访问，清除缓存
    ...
    return () => {
      initializedWebviews.forEach((cardId) => {
        ...
        if (card) {
          ...
          if (webview) {
            // 移除事件监听不要设置opacity
            // webview.style.opacity = '0'; // 移除这行,会导致黑
          }
        }
      });
    };
      ...
      if (index === -1) {
        return [
          ...prev,
          {
            ...
            url: item.url || "",
      title: item.title || existing?.title || "加载...",
            duration: resolvedDuration,
            aid: item.aid || "",
            cid: item.cid || "",
  ...
  const loadCommunitySession = useCallback(async () => {
    ...
    if (session?.user) {
      ...
      const hydratedCards = cardRes.ok ? cardRes.items.map(hydrateCard).filter(Boolean) : [];
  ...
  const handleRevalidateCards = async () => {
    if (!communityMyCards.length) return;
  ...
  useEffect(() => {
    if (!previewUrl) return;
    console.log("[预览] previewUrl:", previewUrl);
        ...
        (() => {
          try {
            ...
            if (!loader) {
              ...
              loader.innerHTML =
                '<span class="rdg-spinner"></span><span class="rdg-loading-text">加载...</span>';
        ...
        <>
          <section className="panel panel-sources">
            <div className="search-type-selector">
              <button
                type="button"
                className={"search-type-btn" + (searchSourceType === "bilibili" ? " is-active" : "")}
                onClick={() => setSearchSourceType("bilibili")}
              >
                B站视频
              </button>
              <button
                type="button"
                className={"search-type-btn" + (searchSourceType === "local" ? " is-active" : "")}
                onClick={() => setSearchSourceType("local")}
              >
                本地视频
              </button>
            </div>

            {searchSourceType === "bilibili" && (
              <>
                ...
                <div className="search-frame">
                  ...
                  {showSearchOverlay ? (
                    <div className="search-overlay">
                      ...
                      <div className="search-overlay-text">加载...</div>
                    </div>
                  ) : null}
                </div>
              </>
              ...
              <>
                <div className="local-folder-section">
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (result?.ok && result.folderPath) {
                        setLocalVideoFolder(result.folderPath);
                  ...
                  {localVideoFolder && (
                    <div className="folder-info">
                      文件夹 {localVideoFolder.split(/[/\\]/).pop()}
                    </div>
                  )}
                </div>
                ...
                {localVideoList.length > 0 && (
                  <div className="local-video-list">
                    <div className="search-hint">找到 {localVideoList.length} 个视频文件</div>
                    <div className="video-file-list">
                      {localVideoList.map((file) => (
                        <div
                          key={file.path}
                          className={"video-file-item" + (selectedLocalFile?.path === file.path ? " is-selected" : "")}
                          onClick={async () => {
                            console.log('点击视频文件:', file);
                            ...
                            try {
                              ...
                              const timeoutPromise = new Promise((_, reject) =>
                                setTimeout(() => reject(new Error('获取元数据超30秒')), 30000)
                              );
                              ...
                              if (metadataResult?.ok && metadataResult.metadata) {
                                ...
                                setLocalVideoInfo(metadata);

                                // 自动设置为activeCard并设置时间范围
                                const duration = metadata.duration || 300; // 如果无法获取时长,默认5分钟
                                setRangeStart(0);
                                setRangeEnd(duration);
                                ...
                                console.error('获取元数据失败', metadataResult);
                                alert(`获取视频信息失败: ${metadataResult?.error || '未知错误'}`);
                              }
                            } catch (error) {
                              console.error('处理视频文件时出错', error);
                              alert(`处理视频文件时出错 ${error.message}`);
                            }
                          }}
                        >
                          <div className="video-file-name">
                            {file.name}
                            {selectedLocalFile?.path === file.path && localVideoInfo === null && (
                              <span className="loading-text"> (解析...)</span>
                            )}
                          </div>
          ...
          <section className="panel panel-preview" key={previewPanelKey}>
            ...
            {activeCard ? (
              <div className="preview-body">
                {activeCard.source === "local" && activeCard.localPath ? (
                  <LocalVideoPlayer
                    key={activeCard.localPath}
                    localPath={activeCard.localPath}
                    localVideoRef={localVideoRef}
                    setDuration={setDuration}
                    setIsPlaying={setIsPlaying}
                    syncCardRange={syncCardRange}
                ...
                {isLoadingPreview || isResolving ? (
                  <div className="preview-loading">
                    ...
                    <div className="preview-loading-text">预览加载...</div>
                  </div>
                ) : null}
                  ...
                  <div className="builder-grid builder-grid--preview">
                    <div className="builder-card">
                      ...
                      <div className="builder-hint">
                        标签来源：{searchSourceType === "bilibili" ? (activeCard ? activeCard.bvid || "未知BV" : "未选择") : (selectedLocalFile?.name || "未选择")}
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div className="builder-grid builder-grid--tags">
                    ...
                    <div className="builder-card">
                      ...
                      <textarea
                        ...
                        placeholder="备注"
                      />
                      <div className="builder-label">可见</div>
                      <select
                      ...
                      {cards.slice(0, 3).map((card) => (
                        <div key={card.id} className="save-item">
                          ...
                          <div className="save-item-meta">
                            {card.bvid} - {formatTime(card.start)}-{formatTime(card.end)}
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        <section className="panel panel-community">
          <div className="manage-layout">
            <aside className="manage-sidebar">
              ...
              <div className="manage-section">
                <div className="manage-section-title">筛</div>
              ...
              <div className="manage-toolbar">
                ...
                <div className="manage-actions">
                  ...
                  <button
                    type="button"
                    className="ghost"
                    onClick={handleRevalidateCards}
                    disabled={!communityMyCards.length || communityStatus.loading}
                  >
                    重新验证
                  </button>
                  <button
                        ...
                        <label className="manage-check">
                          ...
                          <div className="manage-card-info">
                            ...
                            {card.validation && card.validation.status === 'invalid' && (
                              <div
                                className="validation-status validation-status-invalid"
                                onMouseEnter={(e) => {
                                  const rect = e.currentTarget.getBoundingClientRect();
                                  const issues = card.validation.issues || [];
                                  const tooltipText = issues.length > 0 ? issues.join("\n") : "视频源无效";
                                  setTooltip({
                                    visible: true,
                                    text: tooltipText,
                                    x: rect.left,
                          ...
                          {webviewManageIds.has(card.id) ? (
                            <>
                              ...
                              <div
                                ...
                                onMouseDown={(e) => {
                                  e.stopPropagation();
                                  setIsDraggingProgress(true); // 开始拖
              ...
              <div className="builder-grid builder-grid--tags">
                <div className="builder-card">
                  ...
                  <div className="community-search">
                    ...
                    >
                      <option value="latest">最</option>
                      <option value="oldest">最</option>
                    </select>
            ...
            <div className="modal-body">
              <div className="detail-content">
                ...
                <div className="detail-info">
                  ...
                  <div className="detail-section">
                    <div className="detail-label">BV</div>
